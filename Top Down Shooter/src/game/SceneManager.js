...
export default class SceneManager {
  ...
  static moveCamera(x, y){
    this.currentScene.children.filter(i => i.hasComponent("CameraComponent"))[0].x += x;
    this.currentScene.children.filter(i => i.hasComponent("CameraComponent"))[0].y += y;
  }

  static findChild(component){
    return this.currentScene.children.filter(i => i.hasComponent(component));
  }
}