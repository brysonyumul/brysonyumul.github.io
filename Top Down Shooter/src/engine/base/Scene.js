...
class Scene extends NameableParent {
    ...
    update(collidableType, collisionHelper) {
        ...
        for (let i = 0; i < collidableChildren.length; i++) {
            let layerA = collidableChildren[i].collider.layer;

            for (let j = i + 1; j < collidableChildren.length; j++) {
                let layerB = collidableChildren[j].collider.layer;

                if((layerA != layerB) || ((layerA == 99) && (layerA == layerB))){
                    if(!(layerA == 99 && layerB == 98)){
                        if (collisionHelper.inCollision(collidableChildren[i], collidableChildren[j])) {
                            ...
                            //Now loop ove all the behaviors too see if any are listening for collision events
